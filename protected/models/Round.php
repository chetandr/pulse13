...
class Round extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('event, particpants, name', 'required'),
			array('event, particpants', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>256),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, event, particpants, name', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('particpants',$this->particpants);
